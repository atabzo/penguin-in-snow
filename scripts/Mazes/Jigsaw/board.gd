	...
	for r in range(grid_size):
		...
		for c in range(grid_size):
			...
			if (value == grid_size*grid_size):
				...
				tile.set_sprite(value-1, grid_size, tile_size)
...
func is_board_solved():
	...
	for r in range(grid_size):
		for c in range(grid_size):
			if board[r][c] != count:
				if r == c and c == grid_size - 1 and board[r][c] == 0:
					return true
				else:
					return false
			count += 1
...
# testing
func _ready():
	tile_size = floor(size.x / grid_size)
	gen_board()
...
func scramble_board():
	...
	# generate a flat board with values 0 to grid_size*grid_size - 1
	var temp_flat_board = []
	for i in range(grid_size * grid_size - 1, -1, -1):
	...
	for r in range(grid_size):
		for c in range(grid_size):
			board[r][c] = temp_flat_board[r * grid_size + c]
			if board[r][c] != 0:
...
func reset_board():
	...
	for r in range(grid_size):
		board.append(([]))
		for c in range(grid_size):
			board[r].append(r*grid_size + c + 1)
			if r*grid_size + c + 1 == grid_size * grid_size:
				board[r][c] = 0
			else:
				set_tile_position(r, c, board[r][c])
	empty = value_to_grid(0)
...
func _process(_delta):
	...
	if is_pressed:
		...
		if (nr == -1 or nc == -1 or nr >= grid_size or nc >= grid_size):
			return
...
func slide_row(row, dir, limiter):
	...
	if dir == 1:
		...
		for i in range(limiter, empty_index):
			result[i + 1] = result[i]
		result[limiter] = 0
	else:
		# slide towards the left
		for i in range(empty_index, limiter, -1):
			result[i - 1] = result[i]
		result[limiter] = 0
...
func update_size(new_size):
	...
	print('updating board size ', grid_size)

	tile_size = floor(get_size().x / grid_size)
	for tile in tiles:
		tile.queue_free()
	tiles = []
	gen_board()
...
func update_background_texture(texture):
	...
	for tile in tiles:
		...
		tile.update_size(grid_size, tile_size)
